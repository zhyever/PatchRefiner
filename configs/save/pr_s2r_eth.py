...
sub_model_student=dict(
    ...
    max_depth=max_depth,
    load_whole=True,
    pretrained='./work_dir/eth/pr_ft_init/checkpoint_72.pth', # refiner part
    pretrain_model=['', './work_dir/eth/coarse_pretrain/checkpoint_24.pth'], # coarse part
    strategy_supervise='whole_image',
    strategy_inference='whole_image',
    strategy_fine_supervise='whole_image',
    strategy_refiner_target='offset_coarse',
...
model=dict(
    ...
    teacher_pretrain='./work_dir/nfs_fixbug/patchfusionv2/checkpoint_36.pth', # teacher ckp is loaded here (old ckp containing everthing)
    mix_loss=True,
    edge_loss_weight=1,
    ranking_weight=0.1,
    ssi_weight=0.1,
...
# train_cfg=dict(max_epochs=6, val_interval=2, save_checkpoint_interval=6, log_interval=100, train_log_img_interval=100, val_log_img_interval=20, val_type='epoch_base', eval_start=0)
train_cfg=dict(max_epochs=12, val_interval=4, save_checkpoint_interval=12, log_interval=10, train_log_img_interval=10, val_log_img_interval=20, val_type='epoch_base', eval_start=0)
