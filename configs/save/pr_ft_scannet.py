...
model=dict(
    ...
    min_depth=min_depth,
    max_depth=max_depth,
    load_whole=True, # load everything in pretrained checkpoint; if False, only load the refiner part. (It will overload the pretrain_model setting)
    pretrained='./work_dir/nfs_fixbug/patchfusionv2/checkpoint_36.pth',
    pretrain_model=['', './work_dir/nfs_scannet/scannet_coarse_pretrain/checkpoint_core_24.pth'], # will load the coarse model
