...
class Trainer:
    ...
    def val_epoch(self):
        ...
        for idx, (batch_indices, batch_data) in enumerate(zip(loader_indices, self.val_dataloader)):
            ...
            # result, log_dict = self.model(mode='infer', cai_mode='m2', process_num=4, **batch_data_collect) # might use test/val to split cases
            # result, log_dict = self.model(mode='infer', cai_mode='r128', process_num=4, **batch_data_collect) # might use test/val to split cases
    ...
    def run(self):
        ...
        if self.config.debug is True and self.config.debug_val is True:
